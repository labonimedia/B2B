...
const getAllPoRetailerToManufacture = catchAsync(async (req, res) => {
    const filter = pick(req.query, ['productBy', 'manufacturerEmail', 'email']);
...
const updateSinglePoRetailerToManufacture = catchAsync(async (req, res) => {
    const updatedCartItem = await poRetailerToManufactureService.updateSinglePoRetailerToManufacture(
      req.params.id,
      req.body
    );
