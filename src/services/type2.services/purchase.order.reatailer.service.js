...
const combinePurchaseOrders = async (wholesalerEmail) => {
  try {
    ...
    // Fetch manufacturer details for the first product in the purchase orders
    const manufacturerEmail = retailerPOs[0].set[0]?.productBy;
    ...
    retailerPOs.forEach((po) => {
      po.set.forEach((item) => {
        ...
        if (setMap.has(key)) {
          ...
          // Add new combination without wholesaler and manufacturer
          const { wholesaler, manufacturer, ...sanitizedItem } = item;
