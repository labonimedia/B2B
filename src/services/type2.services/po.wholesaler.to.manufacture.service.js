...
const generatePOToManufacturer = async (wholesalerEmail, targetManufacturerEmail) => {
  ...
  const createdFromRetailerPoIds = [...new Set(set.flatMap((s) => s.retailerPoLinks.map((link) => link.poId.toString())))];
...
const createPoToManufacturer = async (wholesalerEmail, combinedPOData) => {
  ...
  for (const item of combinedPOData) {
    ...
    const createdFromRetailerPoIds = [...new Set(set.flatMap((s) => s.retailerPoLinks.map((link) => link.poId)))];
    ...
    for (const poId of createdFromRetailerPoIds) {
      await PORetailerToWholesaler.updateOne({ _id: poId }, { $set: { statusAll: 'processing' } });
    }
  }
