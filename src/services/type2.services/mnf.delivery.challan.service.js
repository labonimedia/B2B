...
const { autoForwardQueue } = require('../../utils/queue');
...
const createMnfDeliveryChallan = async (reqBody) => {
  ...
  await PurchaseOrderType2.findOneAndUpdate({ email, poNumber }, { $set: { status: 'processing' } }, { new: true });
...
const getConfirmRequsted = async (orderID) => {
  ...
  const retailers = await Retailer.find({ email: { $in: retailerEmails } }).select('email fullName companyName');
  ...
  try {
    ...
    for (let order of retailerOrders) {
      ...
      if (orderFulfilled) {
        ...
        await PurchaseOrderRetailerType2.updateOne({ poNumber: order.poNumber }, { $set: { statusAll: 'processing' } });
      } else {
