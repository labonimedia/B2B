...
const { Product, Cart, Manufacture, Wholesaler, User} = require('../models');
 ...
 */
const getCartByEmailToPlaceOrder = async (email) => {
  ...
  if (user.role === 'wholesaler' || user.role === 'retailer') {
    wholesaler = await Wholesaler.findOne({ email }).select('fullName companyName email address country state city pinCode mobNumber GSTIN code');
    if (!wholesaler) {
